...
function Db(databaseName, serverConfig, options) {
  ...
  if(!overrideUsedFlag && this.serverConfig != null && typeof this.serverConfig == 'object' && this.serverConfig._isUsed && this.serverConfig._isUsed()) {    
    revlogger.log('alert',"A Server or ReplSet instance cannot be shared across multiple Db instances");
  ...
  try {
    ...
    revlogger.log('error',"Native bson parser not compiled, please compile or avoid using native_parser=true");
  ...
  if(this.serverConfig.options.readPreference == null
    && this.readPreference != null) {
      ...
      //revlogger.log('info',"Read Preference has set to "+JSON.stringify(this.readPreference));
      revlogger.log('info',"Read Preference has set to "+JSON.stringify(this.options.readPreference));
  }
...
function validateDatabaseName(databaseName) {
  if(typeof databaseName !== 'string') {
    revlogger.log('alert',"database name must be a string");
    throw new Error("database name must be a string");
  }
  if(databaseName.length === 0) {
    revlogger.log('alert',"database name cannot be the empty string");
  ...
  for(var i = 0; i < invalidChars.length; i++) {
    if(databaseName.indexOf(invalidChars[i]) != -1) {
      revlogger.log('alert',"database names cannot contain the character "+invalidChars[i]);
...
Db.prototype.open = function(callback) {
  ...
  if(this.openCalled) {
    // Close db
    this.close();
    revlogger.log('alert',"db object already connecting, open cannot be called multiple times");
  ...
  if(self.serverConfig instanceof Server || self.serverConfig instanceof ReplSet || self.serverConfig instanceof Mongos) {
    ...
    self.serverConfig.connect(self, connect_options, function(err, result) {
      ...
      process.nextTick(function() {
        try {
          ...
          revlogger.log('error',"Error while opening db connection "+err);
    ...
    try {
      revlogger.log('alert',"Server parameter must be of type Server, ReplSet or Mongos");
...
Db.prototype.db = function(dbName) {
  ...
  // Create a new instance
  if(!db) {
    revlogger.log('info',"Creating a new db instance");
  ...
  if(options && (options.strict)) {
    self.collectionNames(collectionName, function(err, collections) {
      ...
      if(collections.length == 0) {
        revlogger.log('error',"Collection " + collectionName + " does not exist. Currently in safe mode.");
        ...
        try {
          ...
          revlogger.log('error',"Failed while getting collection object "+err);
...
Db.prototype.eval = function(code, parameters, options, callback) {
  ...
  this.collection(DbCommand.SYSTEM_COMMAND_COLLECTION).findOne(selector, options, function(err, result) {
    ...
    if(result && result.ok == 1) {
      ...
      revlogger.log('error',"eval failed: " + result.errmsg);
...
Db.prototype.logout = function(options, callback) {
  revlogger.log('info',"Logging out the mongo server");
...
var _executeAuthCreateUserCommand = function(self, username, password, options, callback) {
  ...
  if(options.digestPassword != null) {
    revlogger.log('info',"The digestPassword option is not supported via add_user. Please use db.command('createUser', ...) instead for this option.");
  ...
  if(roles.length == 0) {
    revlogger.log('alert',"Creating a user without roles is deprecated in MongoDB >= 2.6");
...
Db.prototype.createCollection = function(collectionName, options, callback) {
  ...
  this.collectionNames(collectionName, function(err, collections) {
    ...
    if(found && options && options.strict) {
      revlogger.log('alert',"Collection " + collectionName + " already exists. Currently in safe mode.");
...
Db.prototype.command = function(selector, options, callback) {
  ...
  this._executeQueryCommand(DbCommand.createDbSlaveOkCommand(this, selector, command_options), options, function(err, results) {
    if(err) {
      revlogger.log('error',"Error while setting slave OK "+err);
      return callback(err, null);
    }
    if(results.documents[0].errmsg) {
      revlogger.log('error',"Error while setting slave OK "+results.documents[0]);
...
var __executeQueryCommand = function(self, db_command, options, callback) {
  ...
  if(!(typeof read == 'object') && read._type == 'ReadPreference') {
    ...
    if(!ReadPreference.isValid(read)) {
      revlogger.log('alert',"Illegal readPreference mode specified, " + read);

      return callback(new Error("Illegal readPreference mode specified, " + read));
    }
  } else if(typeof read == 'object' && read._type == 'ReadPreference') {
    if(!read.isValid()) {
      revlogger.log('alert',"Illegal readPreference mode specified, " + read.mode);
      return callback(new Error("Illegal readPreference mode specified, " + read.mode));
    }
  }
  ...
  if(typeof callback === 'function' && !onAll) {
    ...
    for(var i = 0; i < connections.length; i++) {
      ...
      if(connection == null) {
        revlogger.log('error',"no open connections");
        return callback(new Error("no open connections"));
...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
  ...
  if(this._applicationClosed) {
    revlogger.log('alert',"db closed by application");
  ...
  if(this.serverConfig.isDestroyed()) {
    revlogger.log('alert',"Connection was destroyed by application");
