...
MongoClient.prototype.connect = function(url, options, callback) {
  ...
  MongoClient.connect(url, options, function(err, db) {
    if(err) {
      revlogger.log('emerg',"Error while opening the mongo connection with the specified url " + url," options "+options);

      revlogger.log('emerg',"Error while opening the mongo connection "+err);
...
MongoClient.connect = function(url, options, callback) {
  ...
  if(object.servers.length == 0) {
    revlogger.log('alert',"connection string must contain at least one seed host");
      ...
      new Db(object.dbName, __server, {safe:false, native_parser:false}).open(function(err, db) {
        ...
        if(totalNumberOfServers == 0) {
          ...
          if(serverConfig == null) {
            return process.nextTick(function() {
              try {
                revlogger.log('error',"Could not locate any valid servers in initial seed list");
...
var _finishConnecting = function(serverConfig, object, options, callback) {
  ...
  revlogger.log('info',"Safe mode write Options"+JSON.stringify(safe));
  ...
  db.open(function(err, db){
    if(err) {
      return process.nextTick(function() {
        try {
          revlogger.log('error',"Error while opening db " + err);
          callback(err, null);
        } catch (err) {
          if(db) db.close();
          throw err
