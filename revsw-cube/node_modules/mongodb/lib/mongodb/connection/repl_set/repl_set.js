...
var ReplSet = exports.ReplSet = function(servers, options) {
  ...
  if(!Array.isArray(servers)) {
    revlogger.log('alert',"The parameter must be an array of servers and contain at least one server");
    throw Error("The parameter must be an array of servers and contain at least one server");
  }
  // Ensure no Mongos's
  for(var i = 0; i < servers.length; i++) {
    if(!(servers[i] instanceof Server)) {
      revlogger.log('alert',"list of servers must be of type Server");

      throw new Error("list of servers must be of type Server");
    }
  }
  ...
  if(this.servers.length == 0) {
    revlogger.log('error',"No valid seed servers in the array");
    throw new Error("No valid seed servers in the array");
  } 
...
ReplSet.prototype.connect = function(parent, options, callback) {
  ...
  this.once("connectionError", function(err, result) {
    revlogger.log('error',"Connect Error " +err);
